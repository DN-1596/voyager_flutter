...
class Intersecter {
  ...
  late StatusLinkedList status_root;
  ...
  Segment segmentCopy(JTS.Coordinate? start, JTS.Coordinate? end, Segment? seg) {
    return Segment(
        ...
        myFill: SegmentFill(above: seg?.myFill.above ?? false, below: seg?.myFill.below),
        otherFill: null);
  }
  ...
  EventNode eventAddSegmentStart(Segment? seg, bool primary) {
    ...
    if (seg?.end != null) eventAdd(ev_start, seg!.end!);
  ...
  EventNode eventAddSegmentEnd(EventNode ev_start, Segment? seg, bool primary) {
    ...
    if (ev_start.pt != null) eventAdd(ev_end, ev_start.pt!);

    return ev_end;
  }

  EventNode eventAddSegment(Segment? seg, bool primary) {
    EventNode ev_start = eventAddSegmentStart(seg, primary);
  ...
  EventNode? checkIntersection(EventNode ev1, EventNode ev2) {
    ...
    JTS.Coordinate a1 = seg1.start!;
    JTS.Coordinate a2 = seg1.end!;
    JTS.Coordinate b1 = seg2.start!;
    JTS.Coordinate b2 = seg2.end!;
    ...
    if (!resultX) {
      ...
      try {
        ...
        log("ERROR!!! in checkIntersection - $e");
      }
    }
    ...
    try {
      while (!event_root.isEmpty) {
        ...
        if (ev.isStart) {
          ...
          Transition surrounding = statusFindSurrounding(ev);
          EventNode? above = surrounding.before;
          EventNode? below = surrounding.after;
          ...
          if (!ev.primary) {
            ...
            SegmentFill? s = ev.seg?.myFill;
            ev.seg?.myFill = ev.seg!.otherFill!;
            ev.seg?.otherFill = s;
          }

          if (ev.seg != null) segments.add(ev.seg!);
        }
      ...
      log("ERROR!!! in calculate Internal - $e");
    }
